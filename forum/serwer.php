...
<?php 
    ...
    if(isset($_POST['ok']) AND isset($_POST['login']) AND isset($_POST['pass'])){   
		...
		$czas   = time(); 
		//$ip1 = mysql_fetch_array(mysql_query("SELECT ip, nick, powod, data_bana, czas_bana FROM forum_ban_ip WHERE ip='$ip'"));
		if($ip1['ip'] == $ip ){
		...
		//ban na nick
		$czas   = time(); 
		//$ban = mysql_fetch_array(mysql_query("SELECT ban FROM forum_konta WHERE login='$login'"));
		if($ban['ban'] >= $czas){
		header("Location: index.php?ban=gracz");
		}
		//dotąd
		$mysql = 1; 
		$mysql['aktywne'] = 1; //= mysql_fetch_array(mysql_query("select ID, aktywne from `forum_konta` where `login` = '$login' AND `password` = '$pass2' limit 1")); 
		//$mysql2 = mysql_fetch_array(mysql_query("select urlop from `forum_konta` where `login` = '$login'")); 
